...
let STRAHLFARBEN = WELTEN.party.strahlen;
function setzeWeltFarben(welt) {
  STRAHLFARBEN = (WELTEN[welt] || WELTEN.party).strahlen;
}
...
class Szene {
  ...
  static plan(auswahl) {
    const { stage, skin, foto, film } = auswahl;
    ...
    if (foto) setz('foto', 'rig', -crewX, 40,
                   `<g transform="scale(.72)">${kameraFigur(foto, 1)}</g>`, 'crew');
    if (film) setz('film', 'rig',  crewX, 40,
  ...
  abgleichen(wunsch) {
    ...
    const BASIS = { trav:0.02, turm:0.10, tube:0.16, sun:0.18, kopf:0.12,
                    laser:0.62, deko:0.24, crew:0.28, mensch:0.32 };
    const SCHRITT = { trav:0.012, turm:0.05, tube:0.04, sun:0.04, kopf:0.035,
                      laser:0, deko:0.03, crew:0.08, mensch:0.025 };
    const zaehler = {};
