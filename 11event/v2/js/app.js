...
/* Welche Welt? Steht in der Adresse: konfigurator.html?welt=hochzeit */
const WELT = WELTEN[new URLSearchParams(location.search).get('welt')] || WELTEN.party;
document.body.dataset.welt = WELT.id;
setzeWeltFarben(WELT.id);
...
const zustand = {
  ...
  foto: null,          /* freiwillig — darf leer bleiben */
  film: null,
  details: { dj: false, stage: false, skin: false, foto: false, film: false },
...
function posten() {
  ...
  const fo = FOTO.find(l => l.id === zustand.foto);
  if (fo) p.push({ art: 'foto', titel: fo.name, unter: `Foto · ${fo.stil}`, preis: fo.preis });
  const fi = FILM.find(l => l.id === zustand.film);
  if (fi) p.push({ art: 'film', titel: fi.name, unter: `Film · ${fi.stil}`, preis: fi.preis });
  return p;
}
...
function schrittleiste() {
  $('#schritte').innerHTML = SCHRITTE.map((s, i) => {
    const fertig = (i === 0 && zustand.dj) || (i === 1 && zustand.stage)
                || (i === 2 && zustand.skin) || (i === 3 && zustand.foto)
                || (i === 4 && zustand.film);
...
function wahlZeichnen() {
  ...
  $('#wahl').innerHTML =
    ...
    s === 'foto'    ? wahlLeute(FOTO, 'foto', 'Foto', 'Wer fotografiert?') :
    s === 'film'    ? wahlLeute(FILM, 'film', 'Film', 'Wer filmt?') :
...
/* Foto und Film sind Zusatz, keine Grundausstattung: hier darf auch
   "niemand" stehenbleiben. Darum Karten statt Stufen. */
function wahlLeute(liste, feld, hut, frage) {
  const gew   = liste.find(l => l.id === zustand[feld]);
  const offen = zustand.details[feld];
  const i     = SCHRITTE.findIndex(s => s.id === feld);
  ...
  return `
    <div class="karte">
      ...
      <div class="leute">
        ${liste.map(l => `
          <button class="mensch" type="button" data-person="${feld}" data-id="${l.id}"
                  aria-pressed="${zustand[feld] === l.id}">
            <span class="mensch__text">
              <b>${l.name}</b>
              <small>${l.stil}</small>
            </span>
            <span class="mensch__preis">${euro(l.preis)}</span>
          </button>`).join('')}
      </div>
      ${gew ? '' : `<p class="fein" style="margin-top:9px">
             Freiwillig — nochmal antippen wählt wieder ab.</p>`}

      ${gew ? `
        <button class="mehr" type="button" data-details="${feld}" aria-expanded="${offen}">
          <span>Über ${gew.name}</span>
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 9l6 6 6-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </button>
        ${offen ? `
          <div class="mehr__inhalt">
            ...
            <dl class="daten">
              ${gew.fakten.map(([k, v]) => `<div><dt>${k}</dt><dd>${v}</dd></div>`).join('')}
            </dl>
          </div>` : ''}` : ''}
...
function rigNeu() {
  szene.baueRig({
    stage: STAGE.find(l => l.id === zustand.stage),
    skin:  SKINS.find(l => l.id === zustand.skin),
    foto:  FOTO.find(l => l.id === zustand.foto),
    film:  FILM.find(l => l.id === zustand.film)
  });
}
...
/* Verlässt man den DJ-Schritt über die Schrittleiste, ohne neu zu
   wählen, muss das Karussell trotzdem zuklappen — sonst bliebe die
   Kamera am einzelnen DJ kleben. */
function karussellSchliessen() {
  if (!zustand.karussell || !zustand.dj) return;
  const i = DJS.findIndex(d => d.id === zustand.dj);
  zustand.djIndex = i;
  zustand.karussell = false;
  szene.waehleDj(i);
  szene.setzeKamera(szene.rahmen(500, 0, -170, 0.52, 470), true);
...
function schrittSetzen(n) {
  ...
  else { karussellSchliessen(); zeichnen(); }
}
...
/* Person wählen — nochmal auf dieselbe tippen wählt sie ab. */
function waehlePerson(feld, id) {
  zustand[feld] = (zustand[feld] === id || !id) ? null : id;
  rigNeu();
  zeichnen();
}
...
function entferne(art) {
  ...
  if (art === 'foto')  waehlePerson('foto', null);
  if (art === 'film')  waehlePerson('film', null);
  if (art === 'dj') {
    zustand.dj = null;
    zustand.stage = null;
    zustand.skin = null;
...
function anfrageSenden() {
  ...
  const url = `mailto:${KONTAKT_MAIL}?subject=${encodeURIComponent(`Anfrage über den Konfigurator (${WELT.label})`)}&body=${encodeURIComponent(text)}`;
...
document.addEventListener('click', (e) => {
  const t = e.target.closest('[data-dj],[data-weiter],[data-zurueck],[data-schritt],[data-punkt],[data-weg],[data-details],[data-stufe],[data-person],#djMehr,#senden');
  if (!t) return;
...
/* ------------------------------------------------------------------
   Start
   ------------------------------------------------------------------ */
/* Die Kopfzeile zeigt zurück in die Welt, aus der man gekommen ist */
$('#markeLink').href = WELT.start;
$('#raus').href = WELT.start;
$('#korbTitel') && ($('#korbTitel').textContent = WELT.anrede);
